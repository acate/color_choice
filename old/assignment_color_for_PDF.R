...
allDataManuallyFixed <- read.csv("~/Google Drive/teaching/CogPsych_4114_2016/in-class_assignments/assignment_2_color/allDataManuallyFixed.csv")
...
cpKSat + geom_point(size=8) + scale_color_identity(guide="none") + coord_fixed() +
  labs(list(title = "Colors constrained to have constant SATURATION", x = "L", y = "U"))
...
cpKVal + geom_point(size=8) + scale_color_identity(guide="none") + coord_fixed() +
  labs(list(title = "Colors constrained to have constant VALUE", x = "L", y = "U"))
